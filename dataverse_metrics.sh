...
export NOW=$(date +'%y%m%d_%H%M%S') #this stores the current date to include in the directory name later
...
curl -o $DIR_PATH/unique_dl_monthly.csv $DATAVERSE_SERVER/$UNIQUE_DL/monthly?parentAlias=UCS-DATA
...
python dataverse_viz.py $DIR_PATH